...
const client = new proto.RateLimitService(
  target,
...
export function checkLimit({ entityId, route, method, tier }) {
  console.log("[grpc-client] Calling CheckLimit");
  console.log({
    target,
    entityId,
    route,
  ...
  return new Promise((resolve) => {
    ...
    client.CheckLimit(
      ...
      (err, response) => {
        if (err) {
          console.warn("[grpc-client] ERROR:", err.message);

          resolve({
            allowed: true,
            remaining: -1,
            retryAfterMs: 0,
            source: "client-fallback-open",
          });
          return;
        }
